...
def pick_theme(topic: str):
    t = (topic or "").lower()
    if any(x in t for x in ["животн","природ","океан","кит","лес","эколог","моря"]): return "nature", THEMES["nature"]
    if any(x in t for x in ["бизнес","компани","продаж","финанс","инвест","стартап"]): return "business", THEMES["business"]
    if any(x in t for x in ["крипт","техно","ai","ии","робот","софт","нейро"]): return "tech", THEMES["tech"]
    if any(x in t for x in ["школ","универ","урок","студент","доклад"]): return "school", THEMES["school"]
    if any(x in t for x in ["мод","стиль","бренд","дизайн","искусств"]): return "fashion", THEMES["fashion"]
...
def can_generate(uid):
    return get_user(uid)["generations"] < PLAN_LIMITS.get(get_user(uid)["plan"], 15)
...
async def ask_grok(prompt: str) -> str:
    try:
        r = await client.chat.completions.create(
            model="grok-3",
            messages=[
                {"role": "system", "content": "Ты арт-директор. Каждый ответ уникален. Не повторяй шаблонные формулировки. Только русский."},
...
def cover(src, dest, w, h):
    ...
    iw, ih = im.size
    if iw / ih > t:
        nw = int(ih * t); x = (iw - nw) // 2; im = im.crop((x, 0, x + nw, ih))
    else:
        nh = int(iw / t); y = (ih - nh) // 2; im = im.crop((0, y, iw, y + nh))
...
def txt(slide, l, t, w, h, text, size, color, bold=False):
    ...
    p.font.color.rgb = RGBColor(*color); p.font.name = "Calibri"
    return box

def main_kb():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📊 Сделать презентацию")],[KeyboardButton(text="📁 Моя история"), KeyboardButton(text="ℹ️ Мой тариф")]], resize_keyboard=True)
def slides_kb():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="5 слайдов"), KeyboardButton(text="8 слайдов")],[KeyboardButton(text="10 слайдов")]], resize_keyboard=True)
def confirm_kb():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="✅ Делать полную версию")],[KeyboardButton(text="➕ Добавить информацию")],[KeyboardButton(text="✏️ Изменить тему")]], resize_keyboard=True)
...
async def process_slides(m: Message, state: FSMContext):
    ...
    sample = await ask_grok(f"Тема: {data.get('topic')}\nУгол подачи: {angle}\nСлайдов: {slides}\nКороткий уникальный план: название и 3 пункта. Не копируй шаблоны. Без JSON.")
    await state.update_data(sample=sample, angle=angle)
...
async def confirm_generate(m: Message, state: FSMContext):
    ...
    uid = m.from_user.id
    u = get_user(uid)
    await m.answer("Собираю уникальную презентацию. Это займёт 1–2 минуты.")
    theme_name, colors = pick_theme(data.get("topic", ""))
    angle = data.get("angle") or random.choice(ANGLES)
    layouts = [0, 1, 2]
    random.shuffle(layouts)

    raw = await ask_grok(f"""Собери уникальную презентацию уровня лучшего журнала или keynote.
Тема: {data.get('topic')}
Слайдов: {data.get('slides')}
Доп: {data.get('extra')}
Угол подачи: {angle}
Не повторяй банальные формулировки. Заголовки разные и живые.
Текст: 2 коротких абзаца на слайд.
Только JSON:
    ...
    for idx, s in enumerate(slides_data):
        ...
        if layout == 0:
            ...
            for i, b in enumerate(blocks or [""]):
                ...
                p.text = b; p.font.size = Pt(16); p.font.color.rgb = RGBColor(*colors["mid"]); p.font.name = "Calibri"; p.space_after = Pt(16)
            ...
            tf = box.text_frame; tf.word_wrap = True
            p = tf.paragraphs[0]; p.text = " ".join((s.get("content") or "").split())
            p.font.size = Pt(15); p.font.color.rgb = RGBColor(*colors["mid"]); p.font.name = "Calibri"
        else:
            ...
            tf = box.text_frame; tf.word_wrap = True
            p = tf.paragraphs[0]; p.text = " ".join((s.get("content") or "").split())
            p.font.size = Pt(16); p.font.color.rgb = RGBColor(*colors["mid"]); p.font.name = "Calibri"
            if img:
