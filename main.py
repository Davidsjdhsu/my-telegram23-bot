...
def append_chat_turn(uid, role: str, content: str):
    """Добавляет одну реплику в память разговора и обрезает до последних
    CHAT_HISTORY_MAX_MESSAGES - вызывать для каждой стороны диалога (и вопрос
    пользователя, и ответ бота), иначе следующий вызов ask_grok_chat() не увидит
    половину контекста."""
    if not content:
        return
    u = get_user(uid)
...
async def handle_vision_upload(m: Message, state: FSMContext, local_path: str, caption: str):
    ...
    try:
        ...
        save_users()
        note_success(uid)
        # Записываем итог в память чата - иначе последующий вопрос вроде "что было на
        # фото" или "объясни второй пункт подробнее" бот бы не понял, у него нет
        # доступа к самому файлу после этого момента, только к тому, что мы ему сохранили.
        append_chat_turn(uid, "user", (caption or "[прислал фото]") + " [приложено фото]")
        append_chat_turn(uid, "assistant", result[:2000])
    except Exception as e:
    ...
    {{"title":"...","slides":[{{"title":"...","content":"абзац1\n\nабзац2","image_prompt":"unique cinematic scene","chart":null}}]}}{grok_json_lang_instruction(cgl or "ru")}""")
        ...
        for idx, s in enumerate(slides_data):
            ...
            if layout == 0:
                ...
                # card_h был 4.65" - рассчитан явно не под реальный объём текста: промпт
                # просит 2-4 предложения на абзац (это максимум ~7-8 строк в узкой колонке
                # ~5" шириной), а карточка была высотой почти во весь слайд. В деле это
                # выглядело как две пустые серые плашки с текстом только в самом верху (см.
                # реальный сгенерированный слайд "Нейлон вытеснил шёлк" - тот же баг, что
                # был у layout 8 со старым row_h=2.3, см. комментарий там). 3.0" с запасом
                # покрывает даже максимум в 4 длинных предложения и не оставляет чугунную
...
async def main():
    ...
    try:
        await bot.set_my_short_description(
            "AlixDoc — презентации, Word, Excel, резюме, договоры и разбор фото задач."
        )
        await bot.set_my_description(
            "Что я умею:\n\n"
            "💬 Отвечаю на вопросы и помогаю с текстом\n"
            "📸 Решаю задачи по фото — тесты, тетради, скриншоты\n"
            "📁 Собираю презентации, Word-документы и Excel-таблицы\n"
            "📝 Резюме и договоры купли-продажи\n"
            "🎨 Могу скопировать стиль вашей презентации\n\n"
